        ...
        <form method="get"  action="{{ route($searchTopBar) }}" class="form-inline frmGetItems float-right" data-toggle="tooltip" title="Buscar...">
            <div class="collapse navbar-collapse navbar-backdrop" id="navbarSearch">
                <div class="d-flex align-items-center pl-4 py-1 py-lg-0">
                    <input type="search" id="search" name="search" value="{{ request('search') }}" class="navbar-input pl-45 pr-3 py-3 py-lg-2 ml-n3 ml-lg-n4 bgc-black-tp7 radius-round border-0 w-100 h-auto autofocus"  placeholder="Buscar por: {{$searchTopBarLabels}}" title="Buscar por: {{$searchTopBarLabels}}">
